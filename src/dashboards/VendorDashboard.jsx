...
const STATUS_MAP = {
  ...
  Cancelled: { bg: T.redLight,     color: T.red,      dot: T.red,        label: "Cancelled" },
  PICKED_UP: { bg: "#f0e8ff",      color: "#5a1a8a",  dot: "#8040e0",    label: "Picked Up" },
};
...
const OrderModal = ({ order, onClose, onUpdateStatus }) => {
  ...
  const nextMap  = { PENDING:"PREPARING", Pending:"PREPARING", PREPARING:"DELIVERED", Preparing:"DELIVERED" };
  const labelMap = { PENDING:"Mark as Preparing", Pending:"Mark as Preparing", PREPARING:"Mark as Delivered", Preparing:"Mark as Delivered" };
      ...
      <div onClick={e => e.stopPropagation()} style={{ background:"white", borderRadius:"24px 24px 0 0", width:"100%", maxWidth:520, maxHeight:"92vh", display:"flex", flexDirection:"column", boxShadow:"0 -8px 40px rgba(0,0,0,.2)", animation:"modalIn .3s cubic-bezier(.34,1.56,.64,1)", overflow:"hidden" }}>
        ...
        {canAdvance && (
          <div style={{ padding:"12px 20px 20px", borderTop:`1px solid ${T.border}`, flexShrink:0, display:"flex", gap:10 }}>
            <button className="btn-danger" onClick={() => { onUpdateStatus(order.id,"CANCELLED"); onClose(); }} style={{ flex:1, padding:"12px" }}>Cancel Order</button>
            <button className="btn-primary" onClick={() => { onUpdateStatus(order.id, nextMap[order.status]); onClose(); }} style={{ flex:2, padding:"12px", fontSize:14 }}>
...
const OverviewTab = ({ vendor, orders, onViewOrder }) => {
  ...
  const todayRev    = todayOrders.filter(o => !["CANCELLED","Cancelled"].includes(o.status)).reduce((a,o) => a+o.total, 0);
  const totalRev    = orders.filter(o => !["CANCELLED","Cancelled"].includes(o.status)).reduce((a,o) => a+o.total, 0);
  const pending     = orders.filter(o => ["PENDING","Pending"].includes(o.status)).length;
  const delivered   = orders.filter(o => ["DELIVERED","Delivered"].includes(o.status)).length;
...
const OrdersTab = ({ orders, onViewOrder, onUpdateStatus }) => {
  ...
  const countFor = f => f==="All" ? orders.length : orders.filter(o => (STATUS_MAP[o.status]?.label||o.status)===f).length;
  const shown = orders
    .filter(o => filter==="All" || (STATUS_MAP[o.status]?.label||o.status)===filter)
      ...
      <div style={{ display:"flex", gap:6, overflowX:"auto", paddingBottom:4, WebkitOverflowScrolling:"touch" }}>
        {filters.map(f => (
          <button key={f} onClick={() => setFilter(f)} className="tab-btn" style={{ padding:"7px 14px", borderRadius:50, border:"none", whiteSpace:"nowrap", background:filter===f?T.green:"white", color:filter===f?"white":T.muted, fontWeight:700, fontSize:12, cursor:"pointer", boxShadow:filter===f?"0 3px 12px rgba(26,92,26,.3)":"none", flexShrink:0 }}>
            {f} <span style={{ opacity:.75 }}>({countFor(f)})</span>
          </button>
        ))}
      </div>
          ...
          : shown.map(o => (
            <div key={o.id} className="card order-row" onClick={() => onViewOrder(o)} style={{ padding:"13px 16px", display:"flex", alignItems:"center", gap:12 }}>
              <div style={{ width:44, height:44, borderRadius:13, background:STATUS_MAP[o.status]?.bg||T.greenLight, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>
                {["DELIVERED","Delivered"].includes(o.status)?"✅":["CANCELLED","Cancelled"].includes(o.status)?"❌":"⏳"}
              </div>
              <div style={{ flex:1, minWidth:0 }}>
