        ...
        >
            <div
                style={{
                    ...
                    animation: "dbIn 0.3s cubic-bezier(.34,1.56,.64,1)",
                    overflow: "hidden",
                }}
              ...
              · {fmtDate(order.date)} at {fmtTime(order.date)}
                ...
                <div style={{ overflowY: "auto", flex: 1, padding: "20px 24px" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
                        {[
                            ["📍 Delivered to", order.deliveryLocation || order.location],
                            ["💳 Payment", order.paymentMethod],
                            ["👤 Customer", order.customerName],
                            ["💰 Total Paid", fmt(order.totalAmount || order.total)],
                        ].map(([label, val]) => (
                    ...
                    {order.groups?.map((group, gi) => (
                        <div key={gi} style={{ marginBottom: 16, background: "#f9fdf9", borderRadius: 14, overflow: "hidden", border: "1px solid #e0eee0" }}>
                            <div style={{ background: "#e8f5e0", padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                ...
                                {group.pack && (
                                    <span style={{ fontSize: 11, color: "#5a8a5a", background: "white", padding: "3px 9px", borderRadius: 20, fontWeight: 600 }}>
                    {group.pack.name} · {fmt(group.pack.price)}
                  </span>
                                )}
                            </div>
                    ...
                    <span style={{ fontSize: 13, color: "#3a5a3a" }}>
                      {item.name} <span style={{ color: "#8aaa8a" }}>×{item.quantity || item.qty}</span>
                    </span>
                                        <span style={{ fontWeight: 700, fontSize: 13, color: "#1a2e1a" }}>{fmt(item.price * (item.quantity || item.qty))}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div style={{ background: "#f4f8f4", borderRadius: 14, padding: "14px 16px" }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            ...
                            <span style={{ fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: 18, color: "#f97316" }}>{fmt(order.totalAmount || order.total)}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        >
            <div
                style={{
                    ...
                    animation: "dbIn 0.28s cubic-bezier(.34,1.56,.64,1)",
                }}
                onClick={(e) => e.stopPropagation()}
            >
                ...
                <div style={{ padding: "14px 22px 20px", flexShrink: 0 }}>
                    <button
                        onClick={() => onSave({ ...form, location: selectedLoc })}
                        style={{
                            width: "100%",
                            padding: "14px",
                            borderRadius: 50,
                            ...
                            fontSize: 15,
                            cursor: "pointer",
                            boxShadow: "0 4px 18px rgba(45,138,45,0.3)",
                        }}
                    >
                        Save Changes
                    </button>
...
// ── DASHBOARD ────────────────────────────────────────────────────────────────
export default function CustomerDashboard({ profile: initialProfile, orders: initialOrders, onBack, onUpdateProfile }) {
    ...
    const totalSpent = orders.reduce((a, o) => a + (o.totalAmount || o.total || 0), 0);
      ...
      `}</style>
            ...
            Chop<span style={{ color: "#f97316" }}>Spot</span>
                ...
                <div style={{ maxWidth: 900, margin: "0 auto", padding: "24px 16px 60px" }}>
                    ...
                    <div
                        style={{
                            background: "linear-gradient(135deg,#1a6a1a 0%,#2d8a2d 50%,#3da03d 100%)",
                            borderRadius: 24,
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(180px,1fr))", gap: 14, marginBottom: 24 }}>
                        {[
                            ...
                            >
                                <div style={{ width: 46, height: 46, borderRadius: 14, background: stat.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0 }}>
                                    {stat.icon}
                                </div>
                                <div>
                    ...
                    >
                                ...
                                {[
                                    ...
                                    <button
                                        ...
                                        style={{
                                            ...
                                            border: "none",
                                            background: activeTab === key ? "#f4f8f4" : "transparent",
                                            color: activeTab === key ? "#2d8a2d" : "#8aaa8a",
                                            fontWeight: activeTab === key ? 700 : 500,
                                            fontSize: 13,
                                            cursor: "pointer",
                                            borderBottom: activeTab === key ? "2px solid #2d8a2d" : "2px solid transparent",
                        ...
                        {loading ? (
                            <div style={{ padding: "80px 20px", textAlign: "center", color: "#8aaa8a" }}>Loading your orders...</div>
                                ...
                                {filteredOrders.map((order, idx) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            style={{
                                                padding: "16px 22px",
                                                borderBottom: idx < filteredOrders.length - 1 ? "1px solid #f0f7f0" : "none",
                                                cursor: "pointer",
                                                transition: "background 0.18s",
                                                display: "flex",
                                                alignItems: "center",
                                        ...
                                        >
                                            ...
                                            <div style={{ flex: 1, minWidth: 140 }}>
                                                ...
                                                <p style={{ margin: "3px 0 0", fontSize: 12, color: "#8aaa8a" }}>
                                                    {fmtDate(order.date)} · {itemCount} item{itemCount !== 1 ? "s" : ""} · {order.deliveryLocation || order.location}
                                                </p>
                                            </div>

                                            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 5, flexShrink: 0 }}>
                                                <span style={{ fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: 15, color: "#1a2e1a" }}>{fmt(order.totalAmount || order.total)}</span>
                                                <span
                                                    style={{
                                                        background: sc.bg,
                                                        color: sc.color,
                          ...
                          <span style={{ width: 6, height: 6, borderRadius: "50%", background: sc.dot, display: "inline-block" }} />
                                            ...
                                            <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#ccc" strokeWidth="2.5" style={{ flexShrink: 0 }}>
                                        ...
                                        >
                                            <div
                                                style={{
                                                    ...
                                                    height: 34,
                                                    borderRadius: 10,
                                                    background: ["#e8f5e0", "#fff5ef", "#f3eeff", "#e3f4fb"][i] || "#f4f8f4",
                                                    display: "flex",
                                                    alignItems: "center",
                                                    justifyContent: "center",
                                                    fontSize: 16,
