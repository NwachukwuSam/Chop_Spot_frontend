...
const OrdersTab = () => {
    ...
    if (loading) return <div style={{ padding: 80, textAlign: "center", color: "#64748b" }}>Loading orders...</div>;
    ...
    return (
        <div style={{ background: "white", borderRadius: 20, overflow: "hidden", border: "1.5px solid #e2e8f0" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                <tr style={{ background: "#f8fafc" }}>
                    <th style={{ padding: "16px 20px", textAlign: "left" }}>Order ID</th>
                ...
                <tbody>
                {orders.map((order, i) => (
                    <tr key={order.id || i} style={{ borderTop: i > 0 ? "1px solid #f1f5f9" : "none" }}>
                        <td style={{ padding: "16px 20px" }}>{order.id}</td>
                        <td style={{ padding: "16px 20px" }}>{order.customer || order.customerName}</td>
                        <td style={{ padding: "16px 20px" }}>{order.vendor || order.vendorName}</td>
                        <td style={{ padding: "16px 20px", textAlign: "right", fontWeight: 700 }}>{fmt(order.totalAmount || order.amount || 0)}</td>
                        <td style={{ padding: "16px 20px", textAlign: "center" }}>
                <span
                    style={{
                        padding: "6px 14px",
...
const UsersTab = () => {
    ...
    useEffect(() => {
        const loadUsers = async () => {
            try {
                setLoading(true);
                // You can add getAllUsers to Api.js if needed
                setUsers([]);
            } catch (err) {
                console.error(err);
            } finally {
...
const ReportsTab = () => (
    <div style={{ background: "white", borderRadius: 20, padding: 40, textAlign: "center", border: "1.5px solid #e2e8f0" }}>
        <p style={{ fontSize: 48, marginBottom: 16 }}>📊</p>
        <h3 style={{ fontFamily: "'Sora',sans-serif", fontWeight: 800 }}>Admin Reports</h3>
        <p style={{ color: "#64748b", marginTop: 12 }}>Detailed platform reports and analytics will be available here.</p>
    </div>
);
            ...
            <div style={{ display: "flex", minHeight: "100vh", background: "#f8fafc", fontFamily: "'DM Sans',sans-serif" }}>
                ...
                >
                    <div style={{ padding: "24px 20px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
                            <div style={{ width: 38, height: 38, borderRadius: 10, background: "linear-gradient(135deg,#3b82f6,#60a5fa)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>🛠️</div>
                            <div>
                                <span style={{ fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: 17, color: "white" }}>Chop<span style={{ color: "#60a5fa" }}>Spot</span></span>
                                <p style={{ margin: 0, fontSize: 10, color: "#64748b", letterSpacing: 1, fontWeight: 700, textTransform: "uppercase" }}>Admin Console</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={{ padding: "14px 16px 20px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            ...
                            <div>
                                ...
                                <p style={{ margin: 0, fontSize: 10, color: "#64748b" }}>Platform Admin</p>
                            </div>
                        </div>
                        {onExit && (
                            <button
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
                    ...
                    <main style={{ flex: 1, padding: "28px 24px 48px", overflowY: "auto", animation: "acIn 0.3s ease both" }} key={tab}>
                        {loading ? (
                            <div style={{ textAlign: "center", padding: "100px 20px", color: "#64748b" }}>Loading Admin Console...</div>
                        ) : (
                            <>
