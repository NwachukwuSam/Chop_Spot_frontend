...
//       <img src={vendor.image} alt={vendor.name} style={{
...
//       <h3 style={{ margin: "0 0 3px", fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 14, color: "#1a2e1a", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{vendor.name}</h3>
...
//         From ₦{vendor.deliveryFrom.toLocaleString()}
//       </div>
...
//     const cartItems = [];
//     vendor.menu.forEach(s => s.items.forEach(item => { if (quantities[item.id] > 0) cartItems.push({ ...item, qty: quantities[item.id] }); }));
...
//                   <span style={{ fontSize: 11, fontWeight: 800, color: "#7aaa7a", letterSpacing: 1, textTransform: "uppercase" }}>🍽 {section.category}</span>
//                 </div>
...
//                     <div>
//                       <p style={{ margin: 0, fontWeight: 600, color: item.available ? "#1a2e1a" : "#bbb", fontSize: 14 }}>{item.name}</p>
//                       <p style={{ margin: "2px 0 0", color: item.available ? "#2d8a2d" : "#ccc", fontWeight: 700, fontSize: 13 }}>₦{item.price.toLocaleString()}</p>
//                     </div>
//                     {item.available ? (
