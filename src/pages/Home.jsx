...
export default function Home() {
    ...
    // ── Load saved delivery profile from localStorage on mount ───────────────
    useEffect(() => {
        try {
            const raw = localStorage.getItem("chopspot_profile");
            if (raw) setSavedProfile(JSON.parse(raw));
        } catch (_) {}
    }, []);

    // ── After returning from login: refresh auth + merge guest cart ───────────
    useEffect(() => {
    ...
    // Auth-gated: guest users are redirected to login before checkout
    ...
    /**
     * Called by PaymentModal after Paystack's onSuccess fires.
     *
     * Flow: Paystack confirms payment → frontend sends reference to backend
     * → backend verifies with Paystack API → order created & marked paid.
     * Order is never created before genuine payment.
     */
    const handleConfirm = async (paystackReference, paymentMethod = "CARD") => {
        ...
        try {
            ...
            const itemsSubtotal = cartGroups.reduce(
                (s, g) => s + g.items.reduce((a, i) => a + i.price * i.qty, 0),
                0
            );
            ...
            console.log("📦 Creating verified order:", orderPayload);
            ...
            toast.error(
                err.message ||
                "Payment received but order failed. Please contact support with your payment reference."
            );
        }
    ...
    if (showDashboard) {
        return (
            <Dashboard
                ...
                onUpdateProfile={p => {
                    setSavedProfile(p);
                    try { localStorage.setItem("chopspot_profile", JSON.stringify(p)); } catch (_) {}
                }}
            />
        );
    }
        ...
        <>
            ...
            <div style={{ position: "relative", zIndex: 1, minHeight: "100vh" }}>
                ...
                <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    style={{ position: "fixed", bottom: 28, left: 28, zIndex: 700, background: "#25D366", borderRadius: 50, padding: "10px 18px", display: "flex", alignItems: "center", gap: 8, cursor: "pointer", boxShadow: "0 4px 16px rgba(37,211,102,0.35)", color: "white", fontWeight: 700, fontSize: 13, fontFamily: "'DM Sans',sans-serif", textDecoration: "none" }}
                >
